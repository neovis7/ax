...
AX가 생성한 스킬의 품질을 검증하고 반복적으로 개선하는 방법론.
Phase 6.1 이후 "스킬 품질 검증" 단계로 실행되며, Phase 6의 구조 검증을 보충하는 레퍼런스.

테스트 아티팩트는 `${PROJECT_DIR}/tests/ax/` 디렉토리에 저장되고, 검증 결과는 `validation-report.json`의 `skill_quality` 필드에 기록된다.
...
| 평가 유형 | 방법 | 적합한 스킬 |
|----------|------|-----------|
| **정성적** | 산출물을 직접 리뷰 | 문체, 디자인, 창작물 등 주관적 품질 |
...
Phase 6.1 구조 검증 완료
  └→ 스킬 품질 검증 (이 가이드)
       └→ 생성된 각 스킬에 대해 With-skill vs Baseline 비교 실행
       └→ assertion 채점 결과를 validation-report.json의 skill_quality 필드에 기록
       └→ 미달 스킬은 Phase 4 에이전트에 수정 요청 후 재검증
  └→ Phase 6.2 시나리오 검증으로 계속
```

검증 실패 기준: 스킬의 `pass_rate`가 0.6 미만이거나, should-trigger 쿼리의 트리거 정확도가 70% 미만.
...
```
"downloads 폴더의 'Q4_매출_최종_v2.xlsx'에서 C열(매출)과 D열(비용)으로
이익률(%) 열을 계산해서 추가하고, 이익률 내림차순으로 정렬해줘."
```

```
"이 PDF 3페이지 표를 CSV로 변환해줘. 헤더가 2줄인데
첫 번째 줄은 카테고리, 두 번째 줄이 실제 열 이름이야."
```

```
"우리 팀 슬랙 채널에서 이번 주 버그 리포트를 모아서
심각도(Critical/High/Medium/Low)별로 분류해줘."
```
...
프롬프트: "{테스트 프롬프트}"
스킬 경로: {스킬 경로}
출력 경로: tests/ax/iteration-N/eval-{id}/with_skill/outputs/
```
...
프롬프트: "{테스트 프롬프트}"  (동일)
스킬: 없음
출력 경로: tests/ax/iteration-N/eval-{id}/without_skill/outputs/
```
...
산출물을 리뷰하고 피드백을 정리한다. AX의 무개입 모드에서는 채점 결과(pass_rate, 실패 assertion)가 피드백을 대신한다.
...
3. **Why를 설명하라** — 피드백이 간결하더라도, 왜 그것이 중요한지 이해하고 그 이해를 스킬에 반영한다.
4. **반복 작업은 번들링하라** — 모든 테스트 실행에서 동일한 헬퍼 스크립트가 생성되면, `scripts/`에 미리 포함한다.

### 6-3. 반복 절차

```
1. 스킬 수정 (Phase 4 에이전트에 SendMessage로 수정 요청)
2. 새 iteration-N+1/ 디렉토리에 모든 테스트 케이스 재실행
3. 채점 결과 비교 (이전 iteration과 pass_rate 비교)
4. 개선이 확인되면 다음 단계로 진행; 미달 시 다시 수정
5. 최대 5회 (execution-policy.json의 max_validation_retries 준수)
```

**종료 조건:**
- 모든 스킬의 `pass_rate` ≥ 0.8
- should-trigger 트리거 정확도 ≥ 80%
- 의미 있는 개선이 더 이상 없음 (연속 2회 pass_rate 변화 < 0.05)
...
AX 컨텍스트에서는 생성된 스킬들 간의 상호 충돌도 확인한다. 동일 프롬프트에 두 스킬이 동시에 트리거되는 경우, 더 구체적인 스킬의 description을 조정한다.
...
테스트/평가 결과는 `${PROJECT_DIR}/tests/ax/` 하위에 체계적으로 관리한다:
...
│   └── evals/
│       └── evals.json
└── scenario-{domain}.md   ← Phase 6.5에서 생성
```

**규칙:**
- eval 디렉토리는 숫자가 아닌 **서술적 이름** 사용 (예: `eval-multi-page-table-extraction`)
- 각 iteration은 독립 디렉토리에 보존 (이전 iteration 덮어쓰기 금지)
- `tests/ax/`는 삭제하지 않음 — 사후 검증 및 감사 추적용

### validation-report.json 스킬 품질 필드

Phase 6.3의 `validation-report.json`에 `skill_quality` 필드를 추가한다:

```json
{
  "timestamp": "{현재 시각}",
  "checks": {
    "agent_frontmatter": "PASS",
    "...": "..."
  },
  "skill_quality": {
    "skills_tested": [
    ...
    "overall": "PASS",
    "failures": ["{미달 스킬명}: pass_rate 0.55 — assertion '출력 파일 존재' 실패"]
  },
  "overall": "PASS",
  "failures": []
}
