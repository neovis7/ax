...
2) 도메인 에이전트 실행 (순차 + 병렬 하이브리드)
   ...
   === Stage B: 계약 검증 게이트 (MUST PASS) ===
   모든 API 엔드포인트를 실제 호출하여 계약 준수 여부를 검증합니다.
   이 게이트를 통과하지 못하면 프론트엔드 에이전트를 시작하지 않습니다.

   절차:
   a) 시드 데이터 삽입 (npm run db:seed)
   b) 서버 시작 + health check
   c) 로그인하여 인증 토큰 획득
   d) api-contracts.ts의 모든 엔드포인트를 순차 호출
   e) 각 응답을 해당 Zod 스키마로 safeParse
   f) 실제 응답 JSON을 .omc/api-snapshots/{endpoint-name}.json에 저장
   g) 결과 판정:
      - 전체 PASS → Stage C로 진행
      - FAIL → 백엔드 에이전트에 수정 지시 (최대 2회 루프)
      - 2회 후에도 FAIL → 사용자 에스컬레이션
