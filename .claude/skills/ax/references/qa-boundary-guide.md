# QA 경계면 가이드 (AX Phase 7 Layer 3 확장)

실제 프로젝트(SatangSlide)에서 발견된 버그 패턴과 그 근본 원인 분석을 바탕으로, AX Phase 7의 3-Layer 검증 체계에서 QA 에이전트가 경계면 결함을 체계적으로 잡는 방법론을 제공한다.

> **적용 범위**: `code+fullstack` 및 `api` 도메인에서만 활성화. visual-qa 이후, 최종 보고 전에 실행.
> **위치**: AX Phase 7 3-Layer 검증의 Layer 3 (E2E) 확장 단계.
> **연동**: `execution-policy.json`의 `on_smoke_failure` 정책에 따라 수정 루프 진입 여부를 결정.

---

## 목차

1. [QA가 놓치는 결함의 패턴](#1-qa가-놓치는-결함의-패턴)
...
6. [실제 사례: SatangSlide 버그 테이블](#6-실제-사례-satangslide에서-발견된-버그)
...
> **AX 컨텍스트**: Layer 1(정적 완성도)과 Layer 2(API 행동)가 통과해도 Layer 3에서 잡아야 할 결함이 이 경계면 불일치다. visual-qa(디자인 검증) 이후, 이 단계에서 통합 정합성을 검증한다.
...
> **AX 컨텍스트**: Phase 3에서 에이전트를 생성할 때 QA 에이전트는 반드시 `type: general-purpose`로 설정. `Explore` 타입 QA 에이전트는 anti-pattern으로 간주.
...
### 3-4. QA는 빌드 후가 아니라, 각 모듈 완성 직후에 실행하라 (증분 QA)
...
> **AX Phase 7 연동**: Phase 7에서 backend-developer 완료 직후 QA 에이전트를 디스패치하고, frontend-developer 완료 후 한 번 더 실행하는 2-단계 증분 QA가 권장 패턴이다. `execution-policy.json`에서 `qa_timing: "incremental"` 설정으로 제어.
...
AX Phase 3에서 QA 에이전트를 생성할 때 포함할 핵심 섹션.
...
AX Phase 7 3-Layer 검증의 Layer 3 확장으로, visual-qa 이후 최종 보고 전에 실행된다.
...
- `on_smoke_failure` 정책 발동 시 수정 루프 입장 (최대 5회)
```

---

## 6. 실제 사례: SatangSlide에서 발견된 버그

이 가이드의 모든 내용은 아래 실제 버그에서 추출한 교훈이다. Layer 1(정적), Layer 2(API 행동)를 통과했음에도 런타임에서 발생한 결함들이다.

| 버그 | 경계면 | 원인 | 해당 검증 섹션 |
|------|--------|------|--------------|
| `projects?.filter is not a function` | API→훅 | API가 `{projects:[]}` 반환, 훅이 배열 기대 | 2-1 |
| 대시보드 모든 링크 404 | 파일경로→href | `/dashboard/` 접두사 누락 | 2-2 |
| 테마 이미지 안 보임 | API→컴포넌트 | `thumbnailUrl` vs `thumbnail_url` | 2-1 |
| 테마 선택 저장 안 됨 | API→훅 | select-theme API 존재, 훅 없음 | 2-4 |
| 생성 페이지 영원히 대기 | 상태전이→코드 | `template_approved` 전이 코드 누락 | 2-3 |
| `data.failedIndices` 크래시 | 즉시응답→프론트 | 백그라운드 결과를 즉시 응답에서 접근 | 2-1 |
| 완료 후 슬라이드 보기 404 | 파일경로→href | `/projects/` → `/dashboard/projects/` | 2-2 |
