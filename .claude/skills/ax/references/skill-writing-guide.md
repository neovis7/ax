...
AX가 생성하는 스킬의 품질을 높이기 위한 상세 작성 가이드.
Phase 4 스킬 생성 절차(`phase-4-skills.md`)의 보충 레퍼런스.

AX 생성 스킬 경로: `${PROJECT_DIR}/.claude/skills/{skill-name}/SKILL.md`
Phase 4.1에서 `skill-skeleton.md` 템플릿 기반으로 생성하며, frontmatter에 `origin=ax`, `domain`, `dependencies` 필드를 포함한다.
...
Claude는 자신의 기본 도구로 쉽게 처리할 수 있는 단순 작업에는 스킬을 호출하지 않는 경향이 있다. "이 파일 읽어줘" 같은 단순 요청은 description이 완벽해도 트리거되지 않을 수 있다. 복잡하고 다단계이며 전문적인 작업일수록 스킬 트리거 확률이 높다.

AX가 생성하는 스킬은 도메인별 전문 작업(예: 금융 리포트 생성, CI/CD 파이프라인 분석, 코드 품질 검토)을 대상으로 하므로, description이 해당 전문성과 복잡성을 명확히 드러내야 한다.
...
```yaml
# AX 생성 스킬 예시 — 도메인 전문 작업
description: "분기별 매출 데이터를 분석하여 YoY/MoM 변화율, 이상치,
  성장 드라이버를 자동 식별하는 재무 분석 스킬. 사용자가 매출 리포트,
  수익성 분석, 재무 대시보드를 요청하거나 revenue/profit/margin 관련
  파일을 언급하면 반드시 이 스킬을 사용할 것."
```
...
- `"재무 분석을 도와주는 스킬"` — "도와주는"은 약하고 수동적, 트리거 조건 없음

### AX frontmatter 예시

AX가 생성하는 스킬의 frontmatter에는 `origin`, `domain`, `dependencies`를 포함한다:
...
### AX 스킬에서 자주 필요한 출력 형식

AX가 생성하는 에이전트 팀은 구조화된 데이터를 주고받으므로 JSON 출력 형식을 명시하는 경우가 많다:

```markdown
## 출력 형식
분석 결과는 다음 JSON 구조로 반환한다:

{
  "summary": "한 줄 요약",
  "findings": [
    { "category": "카테고리명", "severity": "high|medium|low", "detail": "상세 설명" }
  ],
...
도메인별 스킬에서 입력/출력 쌍을 명시하면 에이전트가 기대 동작을 명확히 파악한다:

```markdown
## 처리 예시

**예시 1 — 정상 케이스:**
입력: { "revenue": "1,234,567", "period": "2024-Q4" }
출력: { "value": 1234567, "period": "2024Q4", "normalized": true }

**예시 2 — 에러 케이스:**
입력: { "revenue": "N/A", "period": "2024-Q4" }
출력: { "value": null, "period": "2024Q4", "normalized": false, "reason": "non-numeric" }
```
...
대형 스킬은 모든 내용을 SKILL.md 한 파일에 몰아넣지 않는다. 3-tier 로딩 구조를 사용한다:

**Tier 1 (metadata):** frontmatter + description — 트리거 결정에만 사용
**Tier 2 (body):** SKILL.md 본문 — 핵심 규칙과 프로세스
**Tier 3 (references):** `references/*.md` — 상세 스펙, API 문서, 도메인별 규칙
...
└── references/
    ├── revenue.md (매출, 수익 메트릭)
    ├── cost.md (비용 구조, 마진)
    └── forecast.md (예측 모델, 시나리오)
```

사용자가 매출에 대해 물으면 `revenue.md`만 로드.
...
## 이상치 처리
표준 IQR 방법으로 이상치를 식별한다.
**시계열 데이터인 경우**: [timeseries-outlier.md](references/timeseries-outlier.md) 참조
**범주형 데이터인 경우**: [categorical-outlier.md](references/categorical-outlier.md) 참조
```
...
Phase 4.1에서 스킬 생성 시, 도메인 분석(Phase 1) 결과에서 반복 패턴을 감지하면 `scripts/` 디렉토리에 공통 헬퍼를 선제적으로 번들링한다:

```
finance-analyzer/
├── SKILL.md
├── scripts/
│   ├── normalize_currency.py   # 통화 형식 정규화 (3회 이상 반복 예측)
│   └── detect_outliers.py      # 이상치 탐지 (공통 로직)
└── references/
    └── ...
```
...
스킬 간 데이터 교환의 일관성을 위해 표준 스키마를 사용한다. AX가 생성하는 스킬의 테스트/평가에 활용한다.
...
- 스킬 생성 과정의 메타 정보 (테스트 결과, 반복 이력, AX 파이프라인 로그)
- 사용자 대상 설명서 (스킬은 AI 에이전트를 위한 지시서)
- 이미 Claude가 알고 있는 일반적 지식
- AX 내부 상태 파일 (`.omc/ax/*.json`) — 이는 스킬이 아닌 실행 컨텍스트
