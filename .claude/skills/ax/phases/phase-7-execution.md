...
부모(오케스트레이터)는 서브에이전트 디스패치 전후에 진행 상태를 사용자에게 텍스트로 출력합니다.

**1) 서브에이전트 디스패치 시 — 단계별 상태 표시**

각 서브에이전트를 호출할 때마다 다음 형식으로 출력:
```
[순번/총수] 에이전트명 — 작업 설명... (예상 파일 N개)
```

예:
```
[1/8] visual-architect — 디자인 토큰 생성 중... (2개 파일)
[2/8] backend-developer — API 구현 중... (7개 파일)
```

**2) 서브에이전트 완료 시 — 진행률 % 갱신**

서브에이전트가 완료되면 progress.json을 Read하고 전체 진행률을 계산하여 출력:
```
── 진행률: 45% (9/20 파일) ──────────────────
  ✓ visual-architect     2/2   완료
  ▶ backend-developer    4/7   진행 중
  · frontend-developer   0/7   대기
  · visual-qa            0/4   대기
──────────────────────────────────────────────
```
...
**3) 병렬 실행 시 — 완료된 에이전트부터 갱신**

병렬로 디스패치한 경우, 먼저 완료된 에이전트의 결과를 받을 때마다 진행률을 갱신 출력합니다.
모든 병렬 에이전트가 완료되면 통합 상태를 한 번 더 출력합니다.

**4) 최종 완료 시 — 전체 요약**
```
── 완료: 100% (20/20 파일) ──────────────────
  ✓ visual-architect     2/2   완료
  ✓ backend-developer    7/7   완료
  ✓ frontend-developer   7/7   완료
  ✓ visual-qa            4/4   완료 (52/60점)
──────────────────────────────────────────────
에러: 0개 | 경고: 1개 | 총 소요: 약 N분
```

**서브에이전트 프롬프트에 주입할 지시:**
서브에이전트는 작업 완료 시 반환 메시지에 다음을 포함합니다:
```
완료 파일: [파일1, 파일2, ...]
에러: [없음 또는 에러 목록]
요약: 한 줄 요약
```
부모는 이 반환 메시지로 진행률을 계산합니다.
...
6. **진행률 추적 (progress.json)**

   모든 서브에이전트는 작업 시작/완료 시 `.omc/ax/progress.json`을 업데이트합니다.
   이 파일은 컨텍스트 압축 후 복구의 핵심 데이터입니다.
   ...
   ```json
   {
     ...
     "started_at": "{시작 시각}",
     "updated_at": "{최종 갱신 시각}",
     "current_phase": "phase-7-execution",
     "agents": {
       "visual-architect": { "status": "completed", "files": ["globals.css", "tailwind.config.ts"] },
       "backend-developer": { "status": "in_progress", "files": ["src/routes/auth.ts"], "pending": ["src/routes/quiz.ts", "src/routes/questions.ts"] },
       "frontend-developer": { "status": "pending", "files": [], "pending": ["src/app/dashboard/page.tsx", "..."] }
     },
     "completed_files": ["globals.css", "tailwind.config.ts", "src/routes/auth.ts"],
     "pending_files": ["src/routes/quiz.ts", "src/app/dashboard/page.tsx", "..."],
     "errors": [],
     "last_checkpoint": "backend-developer: auth.ts 완료, quiz.ts 진행 중"
   }
   ```

   **서브에이전트 프롬프트에 주입할 진행률 추적 지시:**
   ```
   ## 진행률 추적 (필수)

   작업 시작 시:
   1. .omc/ax/progress.json을 Read하여 현재 진행 상태 확인
   2. 이미 completed_files에 있는 파일은 건너뜀 (중복 생성 방지)
   3. 자신의 status를 "in_progress"로 업데이트

   파일 생성/수정할 때마다:
   1. 파일을 즉시 Write (메모리에 보관하지 않음)
   2. completed_files에 파일 경로 추가
   3. pending에서 해당 파일 제거
   4. updated_at 갱신

   작업 완료 시:
   1. 자신의 status를 "completed"로 업데이트
   2. last_checkpoint에 완료 요약 기록

   에러 발생 시:
   1. errors 배열에 { agent, file, error, timestamp } 추가
   2. status는 "in_progress" 유지 (재시도 가능)
   ```

7. **컨텍스트 압축 후 복구 프로토콜**

   서브에이전트 또는 부모 에이전트의 컨텍스트가 압축된 경우:

   ```
   압축 감지 (이전 메시지가 요약으로 대체됨)
     ↓
   1. .omc/ax/progress.json을 Read
   2. completed_files 목록 확인 → 이미 완료된 작업은 건너뜀
   3. pending_files 목록 확인 → 미완료 작업부터 재개
   4. errors 배열 확인 → 이전 실패 작업 재시도 또는 스킵
   5. last_checkpoint 확인 → 마지막 작업 상태에서 이어서 진행
   6. CLAUDE.md, docs/plan.md, docs/architecture.md를 다시 Read (압축으로 손실된 규칙 복구)
   ```

   **부모(오케스트레이터)의 압축 후 복구:**
   ```
   1. .omc/ax/progress.json Read
   2. 각 에이전트의 status 확인
   3. status="pending" 또는 "in_progress"인 에이전트만 재디스패치
   4. status="completed"인 에이전트는 스킵
   5. 재디스패치 시 "progress.json을 먼저 Read하고 미완료 작업만 진행하라" 지시
   ```
...
### PDF 생성 품질 게이트 (reportlab + Pretendard)

PDF 보고서를 생성하는 모든 도메인(research, document 등)에 적용합니다.
재작업 없이 한번에 깨끗한 PDF를 생성하기 위한 3단계 사전/사후 검증입니다.

**Gate 1: 마크다운 사전 스캔 (PDF 스크립트 작성 전)**

소스 마크다운 파일을 분석하여 문제를 사전에 파악합니다:

```bash
# 1) Pretendard 미지원 유니코드 문자 검출
grep -P '[\x{2500}-\x{257F}\x{2190}-\x{21FF}\x{2580}-\x{259F}\x{2713}\x{2717}\x{2605}\x{2606}\x{25B6}]' outputs/**/*.md

# 2) 출처 패턴 종류 파악 (모두 통합 처리 필요)
grep -c '출처:' outputs/**/*.md

# 3) 테이블 내 bold 마크업 존재 확인
grep -P '\|.*\*\*.*\*\*.*\|' outputs/**/*.md
```

검출 결과에 따라 PDF 스크립트에 다음을 사전 적용:
- **유니코드 치환 맵**: `──`→`-`, `→`→`>`, `├└│`→`|`, `★☆`→`*`, `✓`→`v`, `✗`→`x`
- **출처 패턴 통합 필터**: `> 출처:`, `*출처:*`, `출처:`, `## 출처`, 테이블 형식 출처 → 모두 수집 후 본문에서 제거, 마지막 References 섹션에 통합
- **이스케이프 순서**: 반드시 `_esc(text)` (XML 이스케이프) → `_fmt(text)` (bold 변환) 순서. 역순이면 `<b>` 태그가 리터럴로 노출됨

**Gate 2: 샘플 1페이지 렌더링 (전체 빌드 전)**

전체 문서를 빌드하기 전에, 다음 3가지 요소를 포함한 테스트 PDF(1페이지)를 먼저 생성:
- 표 1개 (bold 셀 포함)
- 차트 이미지 1개
- 본문 텍스트 (유니코드 문자 포함 행)

테스트 PDF를 `pypdf`로 텍스트 추출하여 `<b>`, `</b>`, `──`, `→` 등이 리터럴로 남아있는지 자동 체크.

**Gate 3: 생성 후 자동 검증 (전체 빌드 후)**
...
# 리터럴 HTML 태그 검출
assert "<b>" not in full_text, "HTML 태그 노출"
assert "</b>" not in full_text, "HTML 태그 노출"
# 깨진 유니코드 검출 (tofu 문자)
assert "──" not in full_text, "미지원 유니코드"
# 인라인 출처 잔존 검출
import re
inline_refs = re.findall(r'출처:\s*\[', full_text)
assert len(inline_refs) == 0, f"인라인 출처 {len(inline_refs)}건 잔존"
```

FAIL 시 해당 항목을 자동 수정하고 재빌드 (최대 2회).

**차트 이미지 비율 보존 규칙:**
- `Image(path, width=고정, height=고정)` 사용 금지 — 비율 왜곡 발생
- 반드시 원본 비율 계산: `ImageReader`로 원본 크기 → 가용 폭에 맞춰 height 자동 산출
- matplotlib 차트도 Pretendard 폰트 사용: `fm.fontManager.addfont(path)` + `plt.rcParams['font.family']` 설정

```python
from reportlab.lib.utils import ImageReader
ir = ImageReader(chart_path)
iw, ih = ir.getSize()
target_w = PAGE_W  # 본문 가용 폭
target_h = ih * (target_w / iw)  # 비율 보존
img = Image(chart_path, width=target_w, height=target_h)
```
...
```
1) visual-architect 호출 (Agent, mode: "bypassPermissions", model: "opus")
...
## 7.2.7 non-code 도메인 검증 (document/creative/research/business)

> 적용 조건: `domain_type`이 code가 아닌 경우.

code 도메인의 3-Layer 검증에 대응하는 non-code 도메인의 검증 절차입니다.

**document/presentation 도메인:**
```
1) 파일 존재 확인
   - 산출물 파일이 모두 생성되었는지 Glob으로 확인
   - HTML 파일인 경우: 유효한 HTML 구조인지 확인 (<!DOCTYPE html>, <head>, <body>)
   - 슬라이드인 경우: 페이지 수가 요구사항 이상인지 확인

2) 콘텐츠 품질 검증
   - 빈 섹션이 없는지 확인 (placeholder 텍스트 "Lorem ipsum", "TBD" 검출)
   - 이미지/차트 참조가 실제 파일로 연결되는지 확인
   - 코드 블록이 syntax highlight 가능한지 확인

3) 프레임워크 준수 확인
   - domain-analysis.json의 frameworks.must에 명시된 프레임워크가 산출물에 반영되었는지 확인
   - 예: Diataxis → 문서가 4유형 중 하나로 분류되어 있는지
   - 예: Bloom's Taxonomy → 학습 콘텐츠가 난이도 순서로 배열되어 있는지
```

**creative 도메인:**
```
1) 산출물 파일 존재 확인
2) 시각 품질: visual-qa 45/60 이상 (기존 루브릭 적용)
3) 브랜드 일관성: 색상/폰트/톤앤매너가 전체에서 일관되는지 확인
```

**research 도메인:**
```
1) 산출물 파일 존재 확인
2) 출처 검증: 인용된 URL/출처가 실제 존재하는지 확인 (WebFetch로 접근 가능 여부)
3) 데이터 정합성: 수치/통계의 출처가 명시되어 있는지 확인
```

**business 도메인:**
```
1) 산출물 파일 존재 확인
2) 프레임워크 적용: BMC 9블록, TAM/SAM/SOM 등이 완성되어 있는지 확인
3) 논리적 일관성: 결론이 분석 데이터와 일치하는지 확인
```

검증 실패 시 수정 루프는 code 도메인과 동일하게 §7.2.1 팀 기반 수정 루프 프로토콜을 따릅니다.
...
10. **PDF 품질 게이트** (PDF 생성 도메인):
    - HTML 태그 리터럴 노출 0건 (`<b>`, `</b>` 등이 텍스트로 보이지 않음)
    - Pretendard 미지원 유니코드 깨짐 0건 (tofu/□ 문자 없음)
    - 인라인 출처 잔존 0건 (모든 출처가 References 섹션에 통합)
11. **출처 검증 필수** (research/document 도메인 — 스킵 불가):
    - source-validator 에이전트 실행 완료 (분석가 완료 → source-validator → report-assembler 순서 필수)
    - 출처 공신력 등급 분류 완료 (1급~4급)
    - 4급(블로그/위키) 출처 비율 30% 이하
    - 1·2급(공시/IR/증권사) 출처 비율 30% 이상 (미달 시 분석가에 1급 출처 보강 재지시)
    - References 섹션에 등급 표기: 각 출처 앞에 [1급], [2급], [3급], [4급] 명시
    - 나무위키, 개인 블로그 등 4급 출처는 1·2급 대체 출처로 교체 시도 후 불가 시에만 유지
    - 차트 이미지 비율 왜곡 없음 (원본 종횡비 보존)
    - 차트 폰트 = Pretendard (matplotlib 포함)
