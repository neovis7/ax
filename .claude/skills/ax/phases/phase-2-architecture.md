...
에이전트를 분리할지 통합할지 판단할 때 4가지 축으로 평가합니다:

| 축 | 분리 (별도 에이전트) | 통합 (하나로 합침) |
|----|---------------------|-------------------|
| **전문성** | 영역이 다르면 분리 (보안 vs 성능) | 영역이 겹치면 통합 (프론트엔드 UI + 로직) |
| **병렬성** | 독립 실행 가능하면 분리 (병렬 처리 이점) | 순차 종속이 강하면 통합 고려 |
| **컨텍스트 부담** | 컨텍스트가 무거우면 분리 (대형 코드베이스 분석) | 가볍고 빠르면 통합 |
| **재사용성** | 다른 팀에서도 쓸 수 있으면 분리 (범용 리뷰어) | 이 팀에서만 쓰면 통합 고려 |

**적용 규칙:**
- 4축 중 3개 이상이 "분리"면 → 별도 에이전트
- 4축 중 3개 이상이 "통합"면 → 하나로 합침
- 2:2면 → 전문성과 병렬성에 더 높은 가중치 부여
- 최대 에이전트 수(10개) 초과 시 → 재사용성이 낮은 에이전트부터 통합

**참고**: 실전 팀 구성 예제는 `references/team-examples.md`를 참조합니다.
...
code + fullstack                   → 레시피 2 (풀스택 웹앱 — shadcn/ui + glassmorphism 기본)
code + fullstack + accuracy        → 레시피 4 (데이터 대시보드 — shadcn/ui + glassmorphism 기본)
  ...
  "agents": [
    {
      ...
      "role": "{역할}",
      "model": "{claude-opus-4-6 기본 — 모든 에이전트는 opus 사용}",
