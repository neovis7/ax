...
- **작동 검증 필수**: 코드를 생성하는 것만으로는 완료가 아닙니다. `--execute` 시 code 도메인에서는 반드시 서버를 실행하고, 핵심 플로우가 실제로 작동하는지 검증해야 합니다. 에러 발생 시 원인을 분석하고 수정한 뒤 재검증합니다. 기획에서 빠진 기능은 허용하지만, 구현하기로 한 기능이 동작하지 않는 상태로 완료 처리하는 것은 금지합니다. 검증 루프는 모든 핵심 플로우가 PASS하거나 최대 3회 반복 후 사용자에게 에스컬레이션할 때까지 계속됩니다.
...
**PPT 스타일 선택 (인터뷰 없이 --execute 시):**

인터뷰를 거치지 않은 경우, Phase 7 시작 시 다음 스타일 선택 질문을 표시합니다:

```
프레젠테이션 스타일을 선택하세요:

  추천:
  테크 제품 → A) glassmorphism / C) gradient-modern
  프리미엄  → B) dark-premium / F) editorial
  스타트업  → C) gradient-modern / G) minimal-swiss
  경영진    → B) dark-premium / H) keynote
  크리에이티브 → D) neo-brutalist / C) gradient-modern
  데이터/분석 → G) minimal-swiss / E) 3d-isometric

  A) glassmorphism   B) dark-premium    C) gradient-modern
  D) neo-brutalist   E) 3d-isometric    F) editorial
  G) minimal-swiss   H) keynote

  선택 (A~H):
```

선택된 스타일은 프레젠테이션 플랜 JSON의 `style` 필드와 `style_guidelines`에 반영됩니다.
이 질문은 무중단 원칙의 예외입니다 — 결과물 품질에 직접 영향을 주는 디자인 결정이므로.
