...
| deer-flow | `npx skills add bytedance/deer-flow` | document+presentation (기본) | 자동 |
...
| 스킬 | 설치 방법 | 용도 |
|------|----------|------|
| deer-flow | `npx skills add bytedance/deer-flow` | PPT/슬라이드 생성 기본 스킬 — 리서치+구조+빌드 파이프라인 (자동 적용 티어) |
...
| **프레젠테이션** | PPT/슬라이드 생성 파이프라인 (이미지 생성 → PPTX 조합). 프로젝트당 1개 | deer-flow (ppt-generation) |
...
### 레시피 3: 프레젠테이션 / 슬라이드
```
필수: deer-flow (자동 적용 — 리서치+구조+빌드 파이프라인)
권장: + supanova-design (프리미엄 비주얼) + better-icons
선택: + make-interfaces-feel-better (전환 효과)
효과: deer-flow가 콘텐츠 구조화 + supanova가 시각 품질 + 풍부한 아이콘
적용: document 도메인, domain_sub_type=presentation, output_format=html
```
...
### document 도메인 (프레젠테이션/슬라이드)
추천 레시피: 레시피 3 (프레젠테이션)
추천 우선순위: deer-flow > supanova-design > better-icons > make-interfaces-feel-better
최소 추천: 1개 (deer-flow 기본)
참고: deer-flow가 콘텐츠 파이프라인, supanova-design이 시각 품질 담당 — 역할 분리로 충돌 없음
...
4. 이미 설치된 스킬은 제외 (Glob으로 `.claude/plugins/` 확인)
5. 충돌 매트릭스 확인 — 충돌 스킬은 하나만 추천
6. **자동 적용 티어 확인**: 자동 적용 조건 충족 시, GitHub URL에서 SKILL.md를 WebFetch
7. fetch된 디자인 스킬 콘텐츠를 `${PROJECT_DIR}/.omc/ax/design-skill-context.md`에 저장
8. Phase 7에서 우선순위 규칙에 따라 에이전트 프롬프트에 주입
9. 완료 보고에 다음을 포함:
   - 적용된 레시피명
   - 자동 적용된 디자인 스킬명
