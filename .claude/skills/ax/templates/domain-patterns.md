...
- `frontend-developer` — UI/UX 구현 (shadcn/ui + glassmorphism 기본)
- `code-reviewer` — 코드 리뷰 + 보안 체크

**UI 기본값 (fullstack/api — `--design none` 미지정 시 자동 적용):**
- **shadcn/ui**: `npx shadcn@latest init` → 컴포넌트 라이브러리 기본 포함. frontend-developer가 Button, Card, Dialog, Table 등 shadcn 컴포넌트를 우선 사용.
- **glassmorphism**: visual-architect가 디자인 토큰에 `backdrop-filter: blur()`, `background: rgba()`, `border: 1px solid rgba()` 글래스모피즘 변수를 기본 포함.
- 사용자가 다른 UI 라이브러리를 명시하면 해당 라이브러리 우선 (shadcn 비활성화).
- `--design none` 명시 시 glassmorphism 비활성화 (shadcn은 유지).
