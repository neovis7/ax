# Domain Frameworks Integration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 도메인별 업계 표준 프레임워크를 Phase 1에서 자동 감지하고, Phase 3에서 에이전트 프롬프트에 주입하여 전문가 수준 산출물을 보장한다.

**Architecture:** 신규 참조 파일(`references/domain-frameworks.md`)에 10개 도메인의 필수/권장 프레임워크를 정의. Phase 1에서 매칭 + 사용자 질문, Phase 3에서 에이전트 `<Process>`/`<Quality_Gates>`에 주입. SKILL.md에 조건부 참조 추가.

**Tech Stack:** 마크다운 참조 파일, phase-1-analysis.md, phase-3-agents.md, SKILL.md

**Spec:** `docs/superpowers/specs/2026-03-28-domain-frameworks-design.md`

---

### File Map

- **Create:** `.claude/skills/ax/references/domain-frameworks.md` — 프레임워크 매핑 + 주입 규칙
- **Modify:** `.claude/skills/ax/phases/phase-1-analysis.md` — 1.5 뒤에 1.5.1~1.5.2 추가
- **Modify:** `.claude/skills/ax/phases/phase-3-agents.md` — 3.2에 프레임워크 주입 규칙 추가
- **Modify:** `.claude/skills/ax/SKILL.md` — 조건부 참조 로드에 domain-frameworks.md 추가

모든 파일은 `/Users/james/Claude/.claude/skills/ax/` 기준.

---

### Task 1: references/domain-frameworks.md 신규 생성

**Files:**
- Create: `/Users/james/Claude/.claude/skills/ax/references/domain-frameworks.md`
...
Read로 파일이 생성되었는지 확인. 필수 테이블에 10개 도메인이 모두 포함되었는지, 권장 테이블에 각 도메인의 권장 프레임워크가 있는지 확인.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/ax/references/domain-frameworks.md
git commit -m "docs(ax): 도메인 프레임워크 매핑 참조 파일 신규 생성"
```

---

### Task 2: Phase 1에 프레임워크 감지 + 사용자 질문 추가

**Files:**
- Modify: `/Users/james/Claude/.claude/skills/ax/phases/phase-1-analysis.md`

- [ ] **Step 1: 1.5 시그널 맵 생성 섹션 뒤에 1.5.1~1.5.2 삽입**

`## 1.6 출력 계약 검증` 앞에 다음 두 섹션을 삽입:
...
- [ ] **Step 2: 1.6 출력 계약 검증에 frameworks 필드 추가**

기존 출력 계약 검증(1.6) 항목 목록에 다음을 추가:
...
Read로 1.5.1과 1.5.2 섹션이 1.5와 1.6 사이에 올바르게 삽입되었는지 확인.

- [ ] **Step 4: Commit**

```bash
git add .claude/skills/ax/phases/phase-1-analysis.md
git commit -m "feat(ax): Phase 1에 도메인 프레임워크 감지 + 사용자 선택 추가"
```

---

### Task 3: Phase 3에 프레임워크 주입 규칙 추가

**Files:**
- Modify: `/Users/james/Claude/.claude/skills/ax/phases/phase-3-agents.md`

- [ ] **Step 1: 3.2 섹션의 본문 커스터마이즈 부분에 프레임워크 주입 규칙 추가**

3.2의 `<Process>` 커스터마이즈 설명 뒤에 다음을 추가:
...
그리고 `<Quality_Gates>` 커스터마이즈 설명 뒤에 다음을 추가:
...
Read로 3.2 섹션에 프레임워크 주입 규칙이 <Process>와 <Quality_Gates> 양쪽에 추가되었는지 확인.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/ax/phases/phase-3-agents.md
git commit -m "feat(ax): Phase 3에 도메인 프레임워크 에이전트 주입 규칙 추가"
```

---

### Task 4: SKILL.md에 조건부 참조 로드 추가

**Files:**
- Modify: `/Users/james/Claude/.claude/skills/ax/SKILL.md`

- [ ] **Step 1: 조건부 참조 로드 섹션에 domain-frameworks.md 추가**

기존 `### 조건부 참조 로드` 섹션의 목록에 다음을 추가:
...
Read로 조건부 참조 로드 목록에 domain-frameworks.md가 추가되었는지 확인.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/ax/SKILL.md
git commit -m "docs(ax): SKILL.md 조건부 참조에 domain-frameworks.md 추가"
```
...
- `references/domain-frameworks.md`에 10개 도메인이 모두 필수 테이블에 포함되어 있는지
- `phase-1-analysis.md`에 1.5.1, 1.5.2 섹션이 1.5와 1.6 사이에 존재하는지
- `phase-3-agents.md`에 `<Process>`와 `<Quality_Gates>` 양쪽에 주입 규칙이 있는지
- `SKILL.md`에 `references/domain-frameworks.md` 참조가 있는지

- [ ] **Step 2: 필수/권장 구분 일관성**

Grep으로 "필수"와 "권장"이 references, phase-1, phase-3 세 파일에서 일관되게 사용되는지 확인.
...
```bash
git add .
git commit -m "fix(ax): 도메인 프레임워크 통합 일관성 수정"
```
