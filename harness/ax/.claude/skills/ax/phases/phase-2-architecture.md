...
document + presentation            → 레시피 3 (프레젠테이션) — deer-flow 또는 pptx 중 택 1 (충돌 관계)
document + html (비프레젠테이션)    → 레시피 3 변형 (supanova-design 기본)
document + pptx + domain_sub_type!=presentation → 레시피 7 (오피스 문서 — 기존 pptx 편집/변환 등)
document + docx                    → 레시피 7 (오피스 문서 — docx 스킬)
document + xlsx                    → 레시피 7 (오피스 문서 — xlsx 스킬)
document + pdf                     → 레시피 7 (오피스 문서 — pdf 스킬)
any + output_format ∈ {pptx,xlsx,docx,pdf} → 레시피 7 (도메인 무관, 포맷 기반)
...
3. 레시피의 "권장" 스킬: 설치 여부 확인 (Glob으로 `.claude/plugins/` + `~/.claude/skills/omc-learned/*/SKILL.md` 확인)
...
6. Phase 7에서 우선순위 규칙에 따라 에이전트 프롬프트에 주입:
   ...
   - 2순위: 품질 강화 스킬 (세부 규칙)
   - 3순위: 에셋 스킬 (리소스 활용)
   - 4순위: 파일 포맷 스킬 (오피스 도구 참조)

**D) 오피스 스킬 자동 적용 (레시피 7):**

`output_format` ∈ {pptx, xlsx, docx, pdf} 일 때:
1. 해당 스킬의 SKILL.md를 로컬 경로에서 Read: `~/.claude/skills/omc-learned/{format}/SKILL.md`
2. `${PROJECT_DIR}/.omc/ax/office-skill-context.md`에 저장
3. Phase 7에서 도메인 실행 에이전트 프롬프트에 "도구 참조" 섹션으로 동적 주입 (design-skill-context.md와 동일한 Phase 7 주입 패턴)
4. `document + presentation`이고 deer-flow와 pptx가 동시 감지되면 → 충돌 매트릭스에 따라 하나만 활성화 (사용자 선택 또는 deer-flow 기본)
