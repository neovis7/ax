...
**deer-flow 미설치 시 폴백:**
`.agents/skills/ppt-generation/` 미존재 시 다음 순서로 대안 탐색:
1. Anthropic pptx 스킬 확인: `~/.claude/skills/omc-learned/pptx/SKILL.md` 존재 시 → pptxgenjs 기반 코드 생성 PPTX 출력 (편집 가능한 텍스트/도형 슬라이드). pptx 스킬의 SKILL.md를 `office-skill-context.md`에 저장하고 slide-builder에 주입
2. pptx 스킬도 없으면 → 기존 html-slide-generator 스킬로 HTML 프레젠테이션 생성 (PPTX 대신 HTML 출력)

**office-skill-context.md 주입 (레시피 7 적용 시):**
`${PROJECT_DIR}/.omc/ax/office-skill-context.md`가 존재하면 (Phase 2.4.1 E에서 생성):
- 도메인 실행 에이전트 (report-writer, data-analyst, slide-builder 등) 프롬프트에 "도구 참조" 섹션으로 주입
- 주입 형태: "다음 스킬의 도구와 스크립트를 사용하여 {format} 파일을 생성/편집하세요"
- visual-architect/visual-qa에는 주입하지 않음 (파일 포맷 도구이므로 시각 검증 대상 아님)
- design-skill-context.md와 독립 — 둘 다 존재하면 각각 적절한 대상 에이전트에 주입
